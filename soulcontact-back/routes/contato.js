...
contatosRouter.post("/contatos", async (req, res) => {
    ...
    // error -> objeto com detalhes dos erros de validação. Se tiver tudo OK, o error será nulo
    // value -> são os dados do req.body

    if(error) {
        res.status(400).json({ message: "Dados inválidos.", error: error.details });
        // HTTP 400 é o Bad Request, indica que a requisição tem dados inválidos
        // details vai mostrar quais infos estarão erradas
        return; // se der erro vai parar o processo e não vai executar as funções abaixo, sem inserir o contato se tiver dados inválidos.
    }
